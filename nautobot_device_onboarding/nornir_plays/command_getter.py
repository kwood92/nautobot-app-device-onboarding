...
@close_threaded_db_connections
def _get_set_send_command_timing(nautobot_job, host_name):
    """Determine whether netmiko ``send_command_timing`` should be used for a host.

    A per-host value supplied via a CSV file (stored on ``ip_address_inventory``) takes
    precedence over the job-wide ``set_send_command_timing`` form input, but only when the
    per-host value is truthy.

    Args:
        nautobot_job: The running SSoT job instance.
        host_name: The name of the host being processed.

    Returns:
        The resolved ``set_send_command_timing`` value (defaults to False).
    """
    inventory = getattr(nautobot_job, "ip_address_inventory", None) or {}
    host_values = inventory.get(host_name) or {}
    if host_values.get("set_send_command_timing"):
        return host_values["set_send_command_timing"]
    return getattr(nautobot_job, "set_send_command_timing", False)
...
def netmiko_send_commands(task: Task, command_getter_yaml_data: Dict, command_getter_job: str, logger, nautobot_job):
    ...
    # Determine if Netmiko send_command_timing should be used for this host.
    set_send_command_timing = _get_set_send_command_timing(nautobot_job, task.host.name)
    ...
    for result_idx, command in enumerate(commands):
        ...
        try:
            current_result = task.run(
                ...
                use_timing=set_send_command_timing,
                read_timeout=60,
                **send_command_kwargs,
            )
